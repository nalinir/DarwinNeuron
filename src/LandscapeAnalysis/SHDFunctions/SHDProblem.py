...
import sys
# TO DO:
# 1. Add a label with the type of problem (existing will just be labeled as SHD old)
# 2. Adjust for different model types
sys.path.append("/scratch/nar8991/snn/snn_ann_hybrid")

from class_based_implementation.models import calc_parameters
...
def generate_shd_problem(shd_config: SHDConfig, nb_hidden=128, nb_classes=20, loss_fn='cross_entropy', db_path='data/landscape-analysis.db', model_type='old'):
    ...
    # 1. Remove the old 'problems' table if it exists
    cur.execute("DROP TABLE IF EXISTS problems")
    con.commit()

    # 2. Create the new 'problems' table with the corrected foreign key relationship
